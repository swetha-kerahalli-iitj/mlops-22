...
for n in range(n_cv):
    ...
    x_train, x_test, y_train, y_test = train_test_split(
        data, digits.target, test_size=0.5, shuffle=False,random_state=random_state
    )
    ...
    print("[{}] Running hyper param tuning for {}".format(n,clf_name))
    actual_model_path = tune_and_save(
        clf, x_train, y_train, x_dev, y_dev, h_metric, h_param_comb[clf_name], model_path='./model/'
    )
...
print(results)
print("Arguments --clf_name {},--random_state {}".format(clf_name,random_state))
results_file = open('./results/'+clf_name+'_'+str(random_state)+'.txt','w')
